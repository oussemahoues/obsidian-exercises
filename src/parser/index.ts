...
export const opt        = <T>(parser:Parser<T>):Parser<T|null>=>parser.times(0,1).map((res: T[])=>(res.length===0 ? null : res[0]));
...
export const NUMERIC    = NUMBER.or(DECIMAL).map((value: number):VAL_TYPE=>({ value,type:'number' }));

export const WORD       = keyed(regex(/[\w_/.\-$]+/));
export const UWORD      = keyed(regex(/[\w_/.\-$]+/u));
export const STRING     = regex(/"(.*?)"|'(.*?)'/).map((str: string)=>str.substring(1,str.length-1)).skip(optWhitespace);
export const SWORD      = WORD.or(STRING);
export const STRING_VAL = lazy(():Parser<VAL_TYPE>=>WORD.map((value: string):VAL_TYPE=>({ value,type:'key' })).or(STRING.map((value: string):VAL_TYPE=>({ value,type:'string' }))));
